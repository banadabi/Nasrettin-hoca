...
export interface LiveClientEventTypes {
  ...
  turncomplete: () => void;
  // Emitted for input audio transcription
  inputTranscription: (transcription: { text: string; isFinal: boolean }) => void;
  // Emitted for output audio transcription
...
export class GenAILiveClient extends EventEmitter<LiveClientEventTypes> {
  ...
  // FIX: Override emit to fix type resolution issue where emit is not found on extended class.
  public emit<T extends keyof LiveClientEventTypes>(
    event: T,
  ...
  // FIX: Add overrides for on and off to fix type resolution issues.
    ...
    if (message.serverContent) {
      ...
      if (serverContent.inputTranscription) {
        this.emit('inputTranscription', {
          ...
          // FIX: Cast to `any` to access `isFinal` property, which is missing from the type definition.
          isFinal: !!(serverContent.inputTranscription as any).isFinal,
        });
      }
      if (serverContent.outputTranscription) {
        this.emit('outputTranscription', {
          ...
          // FIX: Cast to `any` to access `isFinal` property, which is missing from the type definition.
          isFinal: !!(serverContent.outputTranscription as any).isFinal,
        });
      }
